...
if __name__ == "__main__":
    LEVEL = "level1"
    if not os.path.exists(benchlist.path + "OUTPUTS/" + LEVEL):
        os.makedirs(benchlist.path + "OUTPUTS/" + LEVEL)
    ######################################-burst analysis-#######################################
    for nv in benchlist.NVLinks:
        ...
        with open(benchlist.path + "OUTPUTS/" + LEVEL + "/burst_analysis_" + nv + ".csv", "w") as file:
        ...
        for bench, k_list in benchlist.kernels_list[suite].items():
            ...
            for kernel in k_list:
                ...
                for nv in benchlist.NVLinks:
                    ...
                    synthetic_path = benchlist.path + suite + "/" + bench + "/ring/" + nv + "/4chiplet/data/synthetic/" + LEVEL + "/" + str(kernel) + "/bookSim_output.txt"
                    output_path = benchlist.path + suite + "/" + bench + "/ring/" + nv + "/4chiplet/output/" + LEVEL + "/" + str(kernel) + "/"
            ...
            with open(benchlist.path + suite + "/" + bench + "/ring/" + "absolute_throughput_" + ".csv", "w") as file:
                ...
                for item in absolute_throughput:
                    file.write(item + "\n")
            with open(benchlist.path + suite + "/" + bench + "/ring/" + "absolute_packet_latency_" + ".csv", "w") as file:
                file.write("kernel,FullSystem,FullSystem,FullSystem,FullSystem,Synthetic,Synthetic,Synthetic,Synthetic\n,NVLink1,NVLink2,NVLink3,NVLink4, NVLink1,NVLink2,NVLink3,NVLink4\n")
                for item in absolute_packet_latency:
                    file.write(item + "\n")
            with open(benchlist.path + suite + "/" + bench + "/ring/" + "absolute_network_latency_" + ".csv", "w") as file:
                file.write("kernel,FullSystem,FullSystem,FullSystem,FullSystem,Synthetic,Synthetic,Synthetic,Synthetic\n,NVLink1,NVLink2,NVLink3,NVLink4, NVLink1,NVLink2,NVLink3,NVLink4\n")
                for item in absolute_network_latency:
                    file.write(item + "\n")
    with open(benchlist.path + "OUTPUTS/" + LEVEL + "/absolute_throughput_error.csv", "w") as file:
        for item in absolute_throughput_err:
            file.write(item + "\n")
    with open(benchlist.path + "OUTPUTS/" + LEVEL + "/absolute_packet_latency_error.csv", "w") as file:
        for item in absolute_packet_latency_err:
            file.write(item + "\n")
    with open(benchlist.path + "OUTPUTS/" + LEVEL + "/absolute_network_latency_error.csv", "w") as file:
        for item in absolute_network_latency_err:
            file.write(item + "\n")
    with open(benchlist.path + "OUTPUTS/" + LEVEL + "/relative_throughput.csv", "w") as file:
        for item in relative_throughput:
            file.write(item + "\n")
    with open(benchlist.path + "OUTPUTS/" + LEVEL + "/relative_packet_latency.csv", "w") as file:
        for item in relative_packet_latency:
            file.write(item + "\n")
    with open(benchlist.path + "OUTPUTS/" + LEVEL + "/relative_network_latency.csv", "w") as file:
